...
const RESET = "\x1b[0m";
class Interface {
  getColoredCard(cardName) {
    // mettre les numéros en couleur comme dans le jeu

    const cardData = packet.get(cardName);
    if (!cardData) return cardName;
    switch (cardData.type) {
      case 'number':
        if (cardName == '0' || cardName === '3' || cardName === '6' || cardName === '7') return colors.magenta(cardName);else if (cardName === '1' || cardName === '12') return colors.grey(cardName);else if (cardName === '2' || cardName === '5' || cardName === '8') return colors.green(cardName);else if (cardName === '4' || cardName === '11') return colors.blue(cardName);else if (cardName === '9') return colors.yellow(cardName);else return colors.red(cardName);
      case 'modifier':
  ...
  async chooseTarget(currentPlayer, players) {
    ...
    const targets = players.filter(p => p.state).map(p => ({
      name: p.name || `Player ${p.player_nb}`,
      value: p
    }));
  ...
  async chooseSecondChanceTarget(currentPlayer, players) {
    ...
    const targets = players.filter(p => p.state && p !== currentPlayer && !p.hasSecondChance()).map(p => ({
      name: p.name || `Player ${p.player_nb}`,
      value: p
    }));
